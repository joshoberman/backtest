backTestETS<-function(employment_series,ets_model,monthCounters,baseDate){
    ...
    for(i in 1:length(backTestBaseDates)){
        ...
        #make model and forecast 24 steps ahead
        ets_back_test_model<-ets(ts(timeSeriesBack,frequency=12),model="ZZA")
        
        ets_back_test_preds<-forecast(ets_back_test_model,h=24)
        ets_back_test_preds<-summary(ets_back_test_preds)
        thePreds<-round(unlist(ets_back_test_preds$`Point Forecast`))
